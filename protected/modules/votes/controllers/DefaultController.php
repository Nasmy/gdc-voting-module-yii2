...
use Yii;
use yii\helpers\Url;
use app\models\VoterLoginForm;

/**
 * Default controller for the `voter` module
 */
class DefaultController extends BaseController
{
    ...
    public function actionLogin()
    {
		// Yohan
		/*
		if (isset($_GET['token']) && $_GET['token'] == 'NTcxNkRURU5aREEzQkJrdWZHQUtSc2NhWFZSWUwzcHNtVQ==') {
			echo "\n<br>Yohan";
		...
		*/
        ...
        if (Yii::$app->user->identity && Yii::$app->user->can('Dashboard.Index')) {
            ...
            if (isset($params['token'])) {
				...
				//if (isset($token) && $token == 'NDIyNVg1UEtTT0EzQkJrdWZHQUtSc2NhWFZSWUwzcHNtVQ==') {
				//	echo "\n<br>token 2: " . $token;
				//	echo "\n<br>decodedToken: " . $decodedToken;
				//}
                ...
                if ($voterLogin->load($loginFormArr) && $voterLogin->login()) {
					//echo "\n<br />If";

                    Yii::$app->appLog->writeLog($sucMsg, ['hashedToken' => $voterLogin->getHashedToken(), 'voterId' => Yii::$app->user->identity->id]);
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($voterLogin->load(Yii::$app->request->post()) && $voterLogin->login()) {
                Yii::$app->appLog->writeLog($sucMsg, ['Token' => $voterLogin->getHashedToken(), 'Voter ID' => Yii::$app->user->identity->id]);
                $this->redirect(['dashboard/index']);
            } else {
                Yii::$app->session->setFlash('error', $errMsg);

...
class AppLogger extends Component
{
    ...
    function writeLog($message, $params = [], $logLevel = self::INFO)
    {
        ...
        switch ($this->logType) {
            ...
            // User activity log.
            // Format: Date|Time|Email|IP|Domain|Log Level|Activity
            case 1:
                ...
                $msg = "{$date}|{$this->email}|{$ip}|{$domain}|{$this->logLevelLabel[$logLevel]}|{$this->action}|{$message}|{$data} \n";

                break;

            // API log.
            // Format: Date|Time|Uniqid|Email|IP|Domain|Log Level|Activity
            case 2:
                ...
                $msg = "{$date}|{$this->uniqid}|{$this->email}|{$ip}|{$domain}|{$this->logLevelLabel[$logLevel]}|{$this->action}|{$message}|{$data} \n";
